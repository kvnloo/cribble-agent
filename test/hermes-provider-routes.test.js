...
const {
  hermesRoots,
  loadHermesProviderRoutes,
  providerFromRow,
} = require("../lib/hermes-provider-routes");

test("Hermes roots honor explicit comma-separated configuration", () => {
  assert.deepEqual(
    hermesRoots({ HERMES_HOME: " /one , /two,/one " }, "/home/test"),
    ["/one", "/two"],
  );
...
test("provider identity prefers the ledger and safely recognizes known endpoints", () => {
  ...
  assert.equal(
    providerFromRow({ provider: "", baseUrl: "https://openrouter.ai/api/v1" }),
    "openrouter",
  );
...
test("Hermes route aggregation uses provider ledger token weights", () => {
  const calls = [];
  const routes = loadHermesProviderRoutes(
    { HERMES_HOME: "/one,/two" },
    {
      readSqliteJsonRowsFn: (path, sql, options) => {
        calls.push({ path, sql, options });
        return path.startsWith("/one")
          ? [
              ...
              { model: "ignored", provider: "", baseUrl: "", totalTokens: 99 },
            ]
          : [
