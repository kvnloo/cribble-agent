...
const { loadHermesProviderRoutes } = require("./hermes-provider-routes");
...
function claudeConfigDirectories(value, { existsSyncFn = existsSync, realpathSyncFn = realpathSync, statSyncFn } = {}) {
  if (typeof value !== "string" || /[\0\r\n]/.test(value)) throw new Error("CRIBBLE_CLAUDE_CONFIG_DIRS contains invalid control characters.");
  const entries = value.split(",").map((entry) => entry.trim()).filter(Boolean);
  if (!entries.length) throw new Error("CRIBBLE_CLAUDE_CONFIG_DIRS must contain at least one directory.");
  const result = [], seen = new Set();
  for (const entry of entries) {
    if (!isAbsolute(entry)) throw new Error("CRIBBLE_CLAUDE_CONFIG_DIRS entries must be absolute paths.");
    if (!existsSyncFn(entry) || (statSyncFn && !statSyncFn(entry).isDirectory()) || (!statSyncFn && existsSyncFn === existsSync && !statSync(entry).isDirectory())) throw new Error(`CRIBBLE_CLAUDE_CONFIG_DIRS must reference an existing directory: ${entry}`);
    const canonical = realpathSyncFn(entry);
...
function loadCcusage(
  ...
  const reportArgs = ["daily", "--json", ...(byAgent ? ["--by-agent"] : []), ...sinceArgs, ...timezoneArgs];
  const args = configuredBinary ? reportArgs : [bundledBinary, ...reportArgs];
...
function loadUsage(env = process.env, options = {}) {
  ...
  const collectionOptions = { ...options, timezone, byAgent: true };
  ...
  if (platform === "win32" && ["wsl-first", "native-first"].includes(mode)) {
    ...
    if (env.CRIBBLE_CLAUDE_CONFIG_DIRS !== undefined) {
      const directories = claudeConfigDirectories(env.CRIBBLE_CLAUDE_CONFIG_DIRS, options);
      const baselinePath = env.CLAUDE_CONFIG_DIR || join(env.HOME || homedir(), ".claude");
      let baselineRealpath = baselinePath;
      try { baselineRealpath = (options.realpathSyncFn ?? realpathSync)(baselinePath); } catch { /* absent baseline */ }
      for (const directory of directories) {
        if (directory === baselineRealpath) continue;
        const report = loadCcusage({ ...env, CLAUDE_CONFIG_DIR: directory }, { ...collectionOptions, byAgent: true });
        reports.push({ daily: (report.daily ?? report.data ?? []).filter((row) => {
          const labels = [row?.agent, ...(Array.isArray(row?.agents) ? row.agents : []), ...(Array.isArray(row?.metadata?.agents) ? row.metadata.agents : [])]
            .map((value) => String(value ?? "").trim().toLowerCase());
          return labels.includes("claude");
        }) });
      }
    }
  }
  ...
  const providerRoutes = (
    options.loadHermesProviderRoutesFn ?? loadHermesProviderRoutes
  )(env, options);
  if (providerRoutes.length) merged.providerRoutes = providerRoutes;
