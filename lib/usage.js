...
function mergeDailyRows(rows) {
  ...
  for (const row of rows) {
    ...
    for (const [provider, totals] of row.providerTotals) {
      ...
      existingTotals.costUsd = roundCost(
        existingTotals.costUsd + totals.costUsd,
      );
...
function providerBreakdownFrom(daily, events = []) {
  ...
  const add = (name, tokens, cost = 0) => {
    const provider = safeText(name, { maxLength: 256 });
    if (!provider) return;
    const current = totals.get(provider) ?? { totalTokens: 0, costUsd: 0 };
    current.totalTokens += Math.max(0, asNumber(tokens));
    current.costUsd = roundCost(current.costUsd + Math.max(0, asNumber(cost)));
    totals.set(provider, current);
  };
  for (const row of daily) {
    const named = row.providerTotals;
    if (named instanceof Map && named.size) {
      for (const [provider, providerTotals] of named) {
        add(provider, providerTotals.totalTokens, providerTotals.costUsd);
      }
      continue;
    }
    const labels = row.providers?.length ? row.providers : row.agents;
    if (labels?.length === 1) add(labels[0], row.totalTokens, row.costUsd);
  }
  for (const event of events) {
    ...
    add(
      event.provider,
      tokens,
      event.billedCostUsd ?? event.totalCost ?? event.costUsd ?? 0,
    );
  }
