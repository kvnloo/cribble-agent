...
function providerUsageFrom(row, providers, totalTokens) {
  const tokensByProvider = new Map();
  if (Array.isArray(row.providerBreakdowns)) {
    for (const breakdown of row.providerBreakdowns) {
      const provider = safeText(breakdown?.provider ?? breakdown?.name, { maxLength: 256 });
      if (!provider) continue;
      tokensByProvider.set(
        provider,
        (tokensByProvider.get(provider) ?? 0) + modelTokensFromBreakdown(breakdown),
      );
    }
    return tokensByProvider;
  }
  if (providers.length === 1) {
    tokensByProvider.set(providers[0], totalTokens);
  }
  return tokensByProvider;
}
...
function normalizeDay(row) {
  ...
  return {
    ...
    models,
    modelTokenTotals,
    providerTokenTotals: providerUsageFrom(row, providers, calculatedTotal),
    inputTokens,
    outputTokens,
    cacheCreationTokens,
    cacheReadTokens,
...
function mergeDailyRows(rows) {
  ...
  for (const row of rows) {
    ...
    if (!existing) {
      merged.set(row.date, {
        ...
        providerTokenTotals: new Map(row.providerTokenTotals),
      });
      continue;
    }
    ...
    for (const [provider, tokens] of row.providerTokenTotals) {
      existing.providerTokenTotals.set(
        provider,
        (existing.providerTokenTotals.get(provider) ?? 0) + tokens,
      );
    }
  ...
  return [...merged.values()].map(
    ({ modelTokenTotals: _modelTokenTotals, providerTokenTotals, ...row }) => ({
      ...row,
      providerTokenTotals,
    }),
  );
...
function providerBreakdownFrom(daily, events = []) {
  const totals = new Map();
  const add = (name, tokens) => {
    const provider = safeText(name, { maxLength: 256 });
    if (!provider) return;
    totals.set(provider, (totals.get(provider) ?? 0) + Math.max(0, asNumber(tokens)));
  };
  for (const row of daily) {
    const named = row.providerTokenTotals;
    if (named instanceof Map && named.size) {
      for (const [provider, tokens] of named) add(provider, tokens);
      continue;
    }
    const labels = row.providers?.length ? row.providers : row.agents;
    if (labels?.length === 1) add(labels[0], row.totalTokens);
  }
  for (const event of events) {
    const tokens =
      asNumber(event.inputTokens) +
      asNumber(event.outputTokens) +
      asNumber(event.cacheCreationTokens) +
      asNumber(event.cacheReadTokens);
    add(event.provider, tokens);
  }
...
function buildSnapshot(raw, { days = 7, now = new Date() } = {}) {
  ...
  const providers = providerBreakdownFrom(daily, events);
  const displayDaily = daily.map(({ providerTokenTotals: _providerTokenTotals, ...row }) => row);
...
function renderSnapshot(
  ...
  const lines = [
    ...
    ...((snapshot.providers ?? [])
      ...
      .flatMap((row, index) =>
        index === 0
          ? ["", "Providers", `  ${row.name.padEnd(14)}${formatInteger(row.totalTokens)}`]
          : [`  ${row.name.padEnd(14)}${formatInteger(row.totalTokens)}`],
      )),
    "",
