...
function appendOllamaEvent(filePath, event, options={}) {
  ...
  if(stat&&(!stat.isFile()||stat.isSymbolicLink()||stat.uid!==BigInt(process.getuid?.()??Number(stat.uid))))throw new Error("Ollama ledger path is not an owned regular file.");
  options.afterInspect?.();
  ...
  const flags=fs.constants.O_WRONLY|fs.constants.O_APPEND|fs.constants.O_CLOEXEC|(fs.constants.O_NOFOLLOW||0)|(stat?0:fs.constants.O_CREAT|fs.constants.O_EXCL);
  let fd; try { fd=f.openSync(filePath,flags,0o600); } catch(error) { if(!stat&&error.code==="EEXIST")throw new Error("Ollama ledger appeared before append."); throw error; }
  try{const opened=f.fstatSync(fd,{bigint:true});if(stat&&identity(stat)!==identity(opened))throw new Error("Ollama ledger changed before append.");if(!opened.isFile()||opened.uid!==BigInt(process.getuid?.()??Number(opened.uid)))throw new Error("Ollama ledger path is not an owned regular file.");try{(options.fchmodSyncFn||options.chmodSyncFn||f.fchmodSync)(fd,0o600);}catch{throw new Error("Could not establish secure ledger permissions.");}const secure=f.fstatSync(fd,{bigint:true});if((secure.mode&0o777n)!==0o600n)throw new Error("Could not establish secure ledger permissions.");const pathNow=f.lstatSync(filePath,{bigint:true});if(identity(secure)!==identity(pathNow))throw new Error("Ollama ledger changed before append.");f.writeSync(fd,data);f.fsyncSync(fd);}finally{f.closeSync(fd);}
}
